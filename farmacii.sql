-- MariaDB dump 10.19  Distrib 10.4.22-MariaDB, for Win64 (AMD64)
--
-- Host: localhost    Database: farmacii
-- ------------------------------------------------------
-- Server version	10.4.22-MariaDB

/*!40101 SET @OLD_CHARACTER_SET_CLIENT=@@CHARACTER_SET_CLIENT */;
/*!40101 SET @OLD_CHARACTER_SET_RESULTS=@@CHARACTER_SET_RESULTS */;
/*!40101 SET @OLD_COLLATION_CONNECTION=@@COLLATION_CONNECTION */;
/*!40101 SET NAMES utf8mb4 */;
/*!40103 SET @OLD_TIME_ZONE=@@TIME_ZONE */;
/*!40103 SET TIME_ZONE='+00:00' */;
/*!40014 SET @OLD_UNIQUE_CHECKS=@@UNIQUE_CHECKS, UNIQUE_CHECKS=0 */;
/*!40014 SET @OLD_FOREIGN_KEY_CHECKS=@@FOREIGN_KEY_CHECKS, FOREIGN_KEY_CHECKS=0 */;
/*!40101 SET @OLD_SQL_MODE=@@SQL_MODE, SQL_MODE='NO_AUTO_VALUE_ON_ZERO' */;
/*!40111 SET @OLD_SQL_NOTES=@@SQL_NOTES, SQL_NOTES=0 */;

--
-- Table structure for table `companie`
--

DROP TABLE IF EXISTS `companie`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!40101 SET character_set_client = utf8 */;
CREATE TABLE `companie` (
  `NUME` varchar(255) NOT NULL,
  `NRTEL` int(11) NOT NULL,
  PRIMARY KEY (`NUME`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
/*!40101 SET character_set_client = @saved_cs_client */;

--
-- Dumping data for table `companie`
--

LOCK TABLES `companie` WRITE;
/*!40000 ALTER TABLE `companie` DISABLE KEYS */;
/*!40000 ALTER TABLE `companie` ENABLE KEYS */;
UNLOCK TABLES;

--
-- Table structure for table `farmacie`
--

DROP TABLE IF EXISTS `farmacie`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!40101 SET character_set_client = utf8 */;
CREATE TABLE `farmacie` (
  `NUME` varchar(255) NOT NULL,
  `ADRESA` varchar(255) NOT NULL,
  `NRTEL` int(11) NOT NULL,
  PRIMARY KEY (`NUME`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
/*!40101 SET character_set_client = @saved_cs_client */;

--
-- Dumping data for table `farmacie`
--

LOCK TABLES `farmacie` WRITE;
/*!40000 ALTER TABLE `farmacie` DISABLE KEYS */;
/*!40000 ALTER TABLE `farmacie` ENABLE KEYS */;
UNLOCK TABLES;

--
-- Table structure for table `medic_pacient`
--

DROP TABLE IF EXISTS `medic_pacient`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!40101 SET character_set_client = utf8 */;
CREATE TABLE `medic_pacient` (
  `CNPP` bigint(20) NOT NULL,
  `CNPM` bigint(20) NOT NULL,
  `VARSTA` int(11) NOT NULL,
  `NUME` varchar(255) NOT NULL,
  PRIMARY KEY (`CNPP`,`CNPM`),
  KEY `CNPM` (`CNPM`),
  CONSTRAINT `medic_pacient_ibfk_1` FOREIGN KEY (`CNPP`) REFERENCES `pacienti` (`CNPP`),
  CONSTRAINT `medic_pacient_ibfk_2` FOREIGN KEY (`CNPM`) REFERENCES `medici` (`CNPM`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
/*!40101 SET character_set_client = @saved_cs_client */;

--
-- Dumping data for table `medic_pacient`
--

LOCK TABLES `medic_pacient` WRITE;
/*!40000 ALTER TABLE `medic_pacient` DISABLE KEYS */;
/*!40000 ALTER TABLE `medic_pacient` ENABLE KEYS */;
UNLOCK TABLES;

--
-- Table structure for table `medicamente`
--

DROP TABLE IF EXISTS `medicamente`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!40101 SET character_set_client = utf8 */;
CREATE TABLE `medicamente` (
  `TRADENAME` varchar(255) NOT NULL,
  `FORMULA` varchar(255) NOT NULL,
  PRIMARY KEY (`TRADENAME`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
/*!40101 SET character_set_client = @saved_cs_client */;

--
-- Dumping data for table `medicamente`
--

LOCK TABLES `medicamente` WRITE;
/*!40000 ALTER TABLE `medicamente` DISABLE KEYS */;
/*!40000 ALTER TABLE `medicamente` ENABLE KEYS */;
UNLOCK TABLES;

--
-- Table structure for table `medici`
--

DROP TABLE IF EXISTS `medici`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!40101 SET character_set_client = utf8 */;
CREATE TABLE `medici` (
  `CNPM` bigint(20) NOT NULL,
  `NUME` varchar(255) DEFAULT NULL,
  `SPECIALITATE` enum('Chirurgie','Cardiologie','Neurologie','Urologie') DEFAULT NULL,
  `EXPERIENTA` int(11) DEFAULT NULL,
  PRIMARY KEY (`CNPM`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
/*!40101 SET character_set_client = @saved_cs_client */;

--
-- Dumping data for table `medici`
--

LOCK TABLES `medici` WRITE;
/*!40000 ALTER TABLE `medici` DISABLE KEYS */;
/*!40000 ALTER TABLE `medici` ENABLE KEYS */;
UNLOCK TABLES;

--
-- Table structure for table `pacienti`
--

DROP TABLE IF EXISTS `pacienti`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!40101 SET character_set_client = utf8 */;
CREATE TABLE `pacienti` (
  `CNPP` bigint(20) NOT NULL,
  `NUME` varchar(255) NOT NULL,
  `ADRESA` varchar(255) NOT NULL,
  `VARSTA` int(11) NOT NULL,
  PRIMARY KEY (`CNPP`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
/*!40101 SET character_set_client = @saved_cs_client */;

--
-- Dumping data for table `pacienti`
--

LOCK TABLES `pacienti` WRITE;
/*!40000 ALTER TABLE `pacienti` DISABLE KEYS */;
/*!40000 ALTER TABLE `pacienti` ENABLE KEYS */;
UNLOCK TABLES;

--
-- Table structure for table `prescriptii`
--

DROP TABLE IF EXISTS `prescriptii`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!40101 SET character_set_client = utf8 */;
CREATE TABLE `prescriptii` (
  `NRP` int(11) NOT NULL,
  `CNPP` bigint(20) NOT NULL,
  `CNPM` bigint(20) NOT NULL,
  `TRADENAME` varchar(255) NOT NULL,
  `TIMP` date NOT NULL,
  `CANTITATE` int(11) NOT NULL,
  PRIMARY KEY (`NRP`),
  KEY `CNPP` (`CNPP`),
  KEY `CNPM` (`CNPM`),
  KEY `TRADENAME` (`TRADENAME`),
  CONSTRAINT `prescriptii_ibfk_1` FOREIGN KEY (`CNPP`) REFERENCES `pacienti` (`CNPP`),
  CONSTRAINT `prescriptii_ibfk_2` FOREIGN KEY (`CNPM`) REFERENCES `medici` (`CNPM`),
  CONSTRAINT `prescriptii_ibfk_3` FOREIGN KEY (`TRADENAME`) REFERENCES `medicamente` (`TRADENAME`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
/*!40101 SET character_set_client = @saved_cs_client */;

--
-- Dumping data for table `prescriptii`
--

LOCK TABLES `prescriptii` WRITE;
/*!40000 ALTER TABLE `prescriptii` DISABLE KEYS */;
/*!40000 ALTER TABLE `prescriptii` ENABLE KEYS */;
UNLOCK TABLES;

--
-- Table structure for table `produce`
--

DROP TABLE IF EXISTS `produce`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!40101 SET character_set_client = utf8 */;
CREATE TABLE `produce` (
  `TRADENAME` varchar(255) NOT NULL,
  `NUME` varchar(255) NOT NULL,
  PRIMARY KEY (`TRADENAME`,`NUME`),
  KEY `NUME` (`NUME`),
  CONSTRAINT `produce_ibfk_1` FOREIGN KEY (`TRADENAME`) REFERENCES `medicamente` (`TRADENAME`),
  CONSTRAINT `produce_ibfk_2` FOREIGN KEY (`NUME`) REFERENCES `companie` (`NUME`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
/*!40101 SET character_set_client = @saved_cs_client */;

--
-- Dumping data for table `produce`
--

LOCK TABLES `produce` WRITE;
/*!40000 ALTER TABLE `produce` DISABLE KEYS */;
/*!40000 ALTER TABLE `produce` ENABLE KEYS */;
UNLOCK TABLES;

--
-- Table structure for table `vanzare`
--

DROP TABLE IF EXISTS `vanzare`;
/*!40101 SET @saved_cs_client     = @@character_set_client */;
/*!40101 SET character_set_client = utf8 */;
CREATE TABLE `vanzare` (
  `NUME` varchar(255) NOT NULL,
  `TRADENAME` varchar(255) NOT NULL,
  `PRET` double NOT NULL,
  PRIMARY KEY (`NUME`,`TRADENAME`),
  KEY `TRADENAME` (`TRADENAME`),
  CONSTRAINT `vanzare_ibfk_1` FOREIGN KEY (`NUME`) REFERENCES `farmacie` (`NUME`),
  CONSTRAINT `vanzare_ibfk_2` FOREIGN KEY (`TRADENAME`) REFERENCES `medicamente` (`TRADENAME`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8mb4;
/*!40101 SET character_set_client = @saved_cs_client */;

--
-- Dumping data for table `vanzare`
--

LOCK TABLES `vanzare` WRITE;
/*!40000 ALTER TABLE `vanzare` DISABLE KEYS */;
/*!40000 ALTER TABLE `vanzare` ENABLE KEYS */;
UNLOCK TABLES;
/*!40103 SET TIME_ZONE=@OLD_TIME_ZONE */;

/*!40101 SET SQL_MODE=@OLD_SQL_MODE */;
/*!40014 SET FOREIGN_KEY_CHECKS=@OLD_FOREIGN_KEY_CHECKS */;
/*!40014 SET UNIQUE_CHECKS=@OLD_UNIQUE_CHECKS */;
/*!40101 SET CHARACTER_SET_CLIENT=@OLD_CHARACTER_SET_CLIENT */;
/*!40101 SET CHARACTER_SET_RESULTS=@OLD_CHARACTER_SET_RESULTS */;
/*!40101 SET COLLATION_CONNECTION=@OLD_COLLATION_CONNECTION */;
/*!40111 SET SQL_NOTES=@OLD_SQL_NOTES */;

-- Dump completed on 2023-11-14  9:53:15
